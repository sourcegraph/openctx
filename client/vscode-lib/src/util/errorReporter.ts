...
export class ErrorReporterController implements vscode.Disposable {
    ...
    public wrapAsyncGenerator<T, R>(
        userAction: UserAction,
        providerMethod: (params: T, opts?: ProviderMethodOptions) => AsyncGenerator<R>,
    ) {
        const getErrorReporter = this.getErrorReporter.bind(this)
        return async function* (params: T, opts?: ProviderMethodOptions): AsyncGenerator<R> {
            ...
            try {
                for await (const value of providerMethod(params, opts)) {
                    errorReporter.onValue(undefined)
                    errorReporter.report()
                    yield value
                }
