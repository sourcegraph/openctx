...
import { type CorpusArchive, createCorpusArchive } from '../src/corpus/archive/corpusArchive.ts'
import {
    type WebCorpusArchiveOptions,
    createWebCorpusArchive,
} from '../src/corpus/archive/web/webCorpusArchive.ts'
...
const ARCHIVE_KINDS: Record<
    ...
    {
        optionsHelp: string
        toOptions?: (value: any) => unknown
        createFn: (options: any) => Promise<CorpusArchive>
    }
> = {
    web: {
...
function usage(): void {
    ...
    for (const kind of Object.keys(ARCHIVE_KINDS).toSorted() as ArchiveKind[]) {
        console.error(
            `    ${path.basename(process.argv[1])} ${kind} '${ARCHIVE_KINDS[kind].optionsHelp}'`
        )
    }
}
...
if (!kind || !ARCHIVE_KINDS[kind]) {
    console.error(
        `Unrecognized archive kind: ${kind} (valid values are: ${Object.keys(ARCHIVE_KINDS).join(', ')})`
    )
    usage()
    process.exit(1)
}
...
console.error(
    `# Archive complete [${Math.round(performance.now() - t0)}ms]: ${archive.docs.length} docs (${(
        ...
        1024 /
        1024
    ).toFixed(1)} MB), content ID: ${archive.contentID}, description ${JSON.stringify(
        archive.description
    )}`
)
