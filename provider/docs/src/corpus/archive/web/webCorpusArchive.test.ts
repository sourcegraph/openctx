...
`,
    ...
    test('respects canonical URLs', async () => {
        ...
        expect(archive.docs).toMatchObject<Omit<Doc, 'text'>[]>([
...
describe('urlHasPrefix', () => {
    test('same url', () =>
        expect(
            urlHasPrefix(new URL('https://example.com/a/b'), new URL('https://example.com/a/b'))
        ).toBe(true))

    test('path prefix', () => {
        expect(urlHasPrefix(new URL('https://example.com/a/b'), new URL('https://example.com/a'))).toBe(
            true
        )
        expect(urlHasPrefix(new URL('https://example.com/a/b'), new URL('https://example.com/a/'))).toBe(
            true
        )
        expect(urlHasPrefix(new URL('https://example.com/a'), new URL('https://example.com/a/'))).toBe(
            true
        )
        expect(urlHasPrefix(new URL('https://example.com/a-b'), new URL('https://example.com/a'))).toBe(
            false
    ...
    test('query', () => {
        expect(
            urlHasPrefix(new URL('https://example.com/a?page=b'), new URL('https://example.com/a'))
        ).toBe(true)
        expect(
            urlHasPrefix(new URL('https://example.com/a/b?page=c'), new URL('https://example.com/a'))
        ).toBe(true)
    })
})
