...
export async function search(
    ...
    const results = Array.from(combinedResults.values()).flatMap(docResults =>
        Array.from(docResults.values())
    )
...
const SEARCH_METHODS: Record<
    ...
    (
        index: CorpusIndex,
        query: Query
    ) => Omit<SearchResult, 'scores'>[] | Promise<Omit<SearchResult, 'scores'>[]>
> = {
    keywordSearch,
    embeddingsSearch,
}
